      ...
      try {
        ...
        await sql.connect(config.config).then((conn) =>
          conn
            .request()
            .query(
              `SELECT TOP 8 DT.TenDoiTac, DT.LoaiAmThuc, DT.Email, TD.MaThucDon, MA.MaMonAn, MA.TenMonAn, MA.Gia, DT.DienThoaiDoiTac, CN.TenChiNhanh, CONVERT(VARCHAR(5), CN.ThoiGianMoCua,108) as 'ThoiGianMoCua', CONVERT(VARCHAR(5), CN.ThoiGianDongCua,108) as 'ThoiGianDongCua', CN.TinhTrangChiNhanh
      ...
      try {
        ...
        await sql.connect(config.config).then((conn) =>
          conn
            ...
            .then((v) => {
              result = v;
              console.log(req.body.ThoiGianMoCua);
            })
            .then(() => conn.close())
        );
        return result;
